...
/**
 * DnsProviders is a hash of countries full of nameservers mapping country and city to nameservers.
 * 
 * @author tdhite
 *
 */
...
public class DnsProviders
{
	...
	public void fromResources(Context ctx)
	{
		...
		{
			int arrayId = res.getIdentifier(providers[i], Constants.ARRAY, ctx.getPackageName());
			try
			{
				String addrs[] = res.getStringArray(arrayId);

				DnsProvider provider = new DnsProvider(providers[i], addrs[0], i*2);
	...
	public void toInternal(Context context, JSONArray jsonProviders)
	{
		...
		int valid = 0;
		int i;
		for (i = 0; i < jsonProviders.length(); i++)
		{
			...
			if ((i % 100) == 0)
			{
				Log.i(TAG, "Added " + i + " providers, continuing ..." + i);
			}
		}
		...
		Log.i(TAG, "Processed " + valid + " valid provider records out of "
				+ i + " total.");
	}
}
